...
- 5분봉, RSI(14) > 85, 24h 거래량 3M USDT 이상만
- 5분마다 반복 스캔
...
INTERVAL_SECONDS = int(os.getenv("ALT_SHORT_INTERVAL_SECONDS", "10"))
...
def run_scan() -> list[tuple[str, float]]:
    """거래량 3M 이상 심볼만 스캔해 RSI > threshold 인 코인 목록 반환."""
    exchange = ExchangeClient()
    ...
    return sorted(overbought, key=lambda x: -x[1])
...
# ----- 텔레그램 (특정 주제로 전송) -----
def _telegram_env():
    """alt-short용 텔레그램 설정. 주제(Topic)에 보내려면 TELEGRAM_ALT_SHORT_TOPIC_ID 필요."""
    bot_token = os.getenv("TELEGRAM_BOT_TOKEN_ALT_SHORT", "").strip()
    chat_id = os.getenv("TELEGRAM_ALT_SHORT_CHAT_ID", "").strip() or os.getenv("TELEGRAM_CHAT_ID", "").strip()
    topic_id_raw = os.getenv("TELEGRAM_ALT_SHORT_TOPIC_ID", "").strip()
...
def send_telegram_to_topic(text: str) -> bool:
    ...
    .env: TELEGRAM_BOT_TOKEN_ALT_SHORT, TELEGRAM_CHAT_ID(또는 TELEGRAM_ALT_SHORT_CHAT_ID), TELEGRAM_ALT_SHORT_TOPIC_ID
    """
...
def format_alert_message(result: list[tuple[str, float]]) -> str:
    """RSI 과매수 스캔 결과를 텔레그램용 HTML 메시지로 포맷"""
    lines = [
        f"<b>🔥 Alt Short (5분봉 RSI &gt; {RSI_THRESHOLD})</b>",
